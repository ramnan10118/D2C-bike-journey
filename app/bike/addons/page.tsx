  ...
  return (
    ...
    >
      <div className="addon-strip flex w-full min-w-0 items-start">
        <div className="flex min-h-0 w-full min-w-0 flex-1 flex-col gap-[var(--space-2)]">
          {badgeRow}
          <div className="flex w-full min-w-0 items-start gap-[var(--space-3)]">
...
export default function SelectAddOnsPage() {
  ...
  const { addons, setAddons, setSheet, plan } = useBikeJourney();
  ...
  return (
    <div className="min-h-screen px-[var(--journey-inline-padding)] pb-[calc(var(--journey-sticky-footer-clearance)+env(safe-area-inset-bottom,0px))] bg-[var(--color-card-elevated-bg)]">
      <MobileHeader
        ...
        onBack={() => router.push("/bike/plan")}
        subtitle="Add-ons are valid for 1 year."
      />

      <div className="flex w-full flex-col items-stretch gap-[var(--space-3)] mt-[var(--journey-header-content-gap)]">
      ...
      <StickyPriceFooter
        ...
        ctaLabel="Continue"
        onCta={() => router.push("/bike/more-details")}
      />
    </div>
  );
}
