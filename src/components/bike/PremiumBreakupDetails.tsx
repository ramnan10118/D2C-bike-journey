            ...
            <>
              ...
              <div className="premium-breakup-addon-list">
                {selectedAddons.map((line) => (
                  <Row key={line.key} variant={variant} left={line.label} right={formatRupees(line.price)} />
                ))}
              </div>
            </>
          ) : null}
        ...
        >
          {isReview ? (
            <div className="flex justify-between gap-4 items-baseline">
              ...
              <Typography variant="body-md" color="primary" weight="semibold" className="shrink-0 tabular-nums">
                {formatRupees(total)}
              </Typography>
            </div>
          ) : (
